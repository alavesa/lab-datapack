# Creator grid: Si + 2 O -> Silica (SiO2). Exact match only.
execute unless score #done lab.var matches 0 run return 0
execute unless score #c_Si lab.var matches 1 run return 0
...
data modify block ~ ~ ~ Items set value [{Slot:0b,id:"minecraft:potion",count:1,components:{"minecraft:custom_name":{text:"Silica",color:"aqua",italic:false},"minecraft:lore":[{text:"SiO2",color:"gray",italic:false},{text:"Lab compound",color:"dark_gray",italic:false},{text:"Not drinkable",color:"dark_gray",italic:true}],"minecraft:custom_data":{lab_compound:"SiO2",lab_is_compound:1b,lab_no_drink:1b},"minecraft:max_stack_size":1,"minecraft:custom_model_data":{strings:["lab_flask"]},"minecraft:consumable":{consume_seconds:3600f,animation:"drink",sound:"minecraft:entity.generic.drink",has_consume_particles:0b},"minecraft:potion_contents":{custom_color:15132410,custom_effects:[]}}}]
summon item ~ ~0.8 ~ {PickupDelay:15,Item:{id:"minecraft:glass",count:2}}
...
title @a[distance=..6] actionbar {"text":"The mixture becomes Silica.","color":"gray"}
