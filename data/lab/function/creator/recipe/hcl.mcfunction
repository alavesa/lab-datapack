# Creator grid: H + Cl -> Hydrochloric Acid (HCl). Exact match only.
execute unless score #done lab.var matches 0 run return 0
execute unless score #c_H lab.var matches 1 run return 0
execute unless score #c_Cl lab.var matches 1 run return 0
execute unless score #total lab.var matches 2 run return 0
scoreboard players set #done lab.var 1
data remove block ~ ~ ~ Items
data modify block ~ ~ ~ Items set value [{Slot:0b,id:"minecraft:potion",count:1,components:{"minecraft:custom_name":{text:"Hydrochloric Acid",color:"aqua",italic:false},"minecraft:lore":[{text:"HCl",color:"gray",italic:false},{text:"Lab compound",color:"dark_gray",italic:false},{text:"Not drinkable",color:"dark_gray",italic:true}],"minecraft:custom_data":{lab_compound:"HCl",lab_is_compound:1b,lab_no_drink:1b},"minecraft:max_stack_size":1,"minecraft:custom_model_data":{strings:["lab_test_tube"]},"minecraft:consumable":{consume_seconds:3600f,animation:"drink",sound:"minecraft:entity.generic.drink",has_consume_particles:0b},"minecraft:potion_contents":{custom_color:13434726,custom_effects:[]}}}]
...
title @a[distance=..6] actionbar {"text":"The mixture becomes Hydrochloric Acid.","color":"gray"}
